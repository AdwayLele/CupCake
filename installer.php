...
$dbSettings = false;
$propelFiles = false;
global $host, $dbName, $userName, $password, $userBaseURL;
...
<link href='site-templates/custom.css' rel='stylesheet' type='text/css' />
...
$userBaseURL = $_POST['baseURL'];

if($host == ''){
	echo "You haven't provided host name, please enter 'host name'.";
	exit;
}
if($dbName == ''){
	echo "You haven't provided database name, please enter 'database name'.";
	exit;
}
if($userName == ''){
	echo "You haven't provided user name, please enter 'user name'.";
	exit;
}
...
$configContent = file_get_contents("$baseURL/application/config/config.php");
$configContent = str_replace("http://localhost/cpu/", $userBaseURL, $configContent);
$configHandle = fopen("$baseURL/application/config/config.php", "w");
...
<label>Base URL:</label><input type = 'text' name = 'baseURL'><br/>
...
if(isset($_GET["install"]))
{
	...
	(7, 'template', '/site-templates/custom.css');
	";
...
{
	header("refresh:3; url = $userBaseURL/installer.php?install=true");
	echo "
	<a href='?install=true'>Click here</a> to create tables, if not being redirected within 3 seconds.
	";
